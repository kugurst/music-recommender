...
def _tempo_model(use_flat=False):
    ...
    if use_flat:
        model.add(keras.layers.Lambda(lambda x: x, input_shape=TEMPO_SHAPE, name=InputNames.TEMPO.get_layer_name()))
        return model
...
def _flux_model(use_flat=False):
    ...
    if use_flat:
        model.add(keras.layers.Lambda(lambda x: x, input_shape=FLUX_SHAPE, name=InputNames.FLUX.get_layer_name()))
        return model
...
def _rolloff_model(use_flat=False):
    ...
    if use_flat:
        model.add(keras.layers.Lambda(lambda x: x, input_shape=ROLLOFF_SHAPE, name=InputNames.ROLLOFF.get_layer_name()))
        return model
...
def _mel_model(use_flat=False):
    ...
    if use_flat:
        # model.add(keras.layers.Lambda(lambda x: x, input_shape=MEL_SHAPE, name=InputNames.MEL.get_layer_name()))
...
def _contrast_model(use_flat=False):
    ...
    if use_flat:
        # model.add(keras.layers.Lambda(lambda x: x, input_shape=CONTRAST_SHAPE,
...
def _tonnetz_model(use_flat=False):
    ...
    if use_flat:
        # model.add(keras.layers.Lambda(lambda x: x, input_shape=TONNETZ_SHAPE, name=InputNames.TONNETZ.get_layer_name()))
...
def _chroma_model(use_flat=False):
    ...
    if use_flat:
        # model.add(keras.layers.Lambda(lambda x: x, input_shape=CHROMA_SHAPE, name=InputNames.CHROMA.get_layer_name()))
...
def _hpss_model(use_flat=False):
    ...
    if use_flat:
        # model.add(keras.layers.Lambda(lambda x: x, input_shape=HPSS_SHAPE, name=InputNames.HPSS.get_layer_name()))
...
def _rms_fractional_model(use_flat=False):
    ...
    if use_flat:
        model.add(keras.layers.Lambda(lambda x: x, input_shape=RMS_SHAPE,
...
class BestPrecisionSaver(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        if val_precision > self.best_precision:
            ...
            print("Validation precision did not improve from [{}]".format(self.best_precision))
...
def train_model_flat(model, train_set, train_target, validate_set, validate_target, epochs=500, batch_size_base=1024):
    ...
    try:
        model.fit(
            ...
            shuffle=True, class_weight={0: 0.25, 1: 1},
